...
class Mainwindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.create_password_thread = Thread(target = self.create_password_thread_handler, args = (self.create_password_event, ))
        self.create_password_event.set()
        self.create_password_thread.start()

        self.find_password_event = Event()
        self.find_password_thread = Thread(target = self.find_password_thread_handler, args = (self.find_password_event, ))
        self.find_password_event.set()
        self.find_password_thread.start()

    def create_password_thread_handler(self, event : Event):
    ...
    def find_password_thread_handler(self, event : Event):
        while self.exit_flag == False:            
            while event.is_set() == False:
                find_time = time.time()
                self.state.ChangeState.emit(False)
                ...
                event.set()
                self.state.ChangeState.emit(True)
    @Slot()
